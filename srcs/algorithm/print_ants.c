...
/*   Updated: 2020/07/07 15:16:30 by gmolin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		print_ants(t_lem *lem_in, t_path *paths)
{
	...
	x = 0;
	i = -1;
	while (arrived_ants < lem_in->ant_amount && ++lem_in->steps)
	{
		current_path = reset_ants(&x, &i, paths);
 		while (++i < paths->max)
		{
			move_ants_on_path(lem_in, current_path->path, &arrived_ants, &x);
			if (lem_in->move_ants < lem_in->ant_amount && paths->division[i] > 0)
			{
				x = check_print_space(x);
				lem_in->move_ants = send_new_ant(lem_in, current_path->path, lem_in->move_ants, &arrived_ants);
				paths->division[i]--;
			}
